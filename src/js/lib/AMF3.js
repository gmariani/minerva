...
AMF3.prototype = {
	...
	writeData: function(ba, node) {
		...
		switch(type) {
			...
			case "False" 			: this.writeBoolean(ba, false); break;
			case "True" 			: this.writeBoolean(ba, true); break;
			...
			case "Array" 			: this.writeArray(ba, node.value); break;
			case "Object" 			: this.writeObject(ba, node.value, node.__traits); break;
			case "XML" 				: this.writeXML(ba, node.value); break;
			case "ByteArray" 		: this.writeByteArray(ba, node.value); break;
			case "Vector.<int>" 	: this.writeVectorInt(   ba, node.value, node.__traits); break;
			case "Vector.<uint>" 	: this.writeVectorUInt(  ba, node.value, node.__traits); break;
			case "Vector.<Number>" 	: this.writeVectorDouble(ba, node.value, node.__traits); break;
			case "Vector.<Object>" 	: this.writeVectorObject(ba, node.value, node.__traits); break;
			case "Dictionary" 		: this.writeDictionary(  ba, node.value, node.__traits); break;
			default: throw Error("Undefined AMF3 type encountered '" + type + "'");
		};
	},
	...
	readXMLDoc: function(ba) {
		...
		var xmldoc = { value:ba.readUTFBytes(ref >> 1), __traits:{ type:'XMLDocument' }};
	...
	readDate: function(ba) {
		...
		var d = { value:new Date(ba.readDouble()), __traits:{ type:'Date' }};
	...
	writeArray: function(ba, value) {
		...
		for (key in value) {
			numElements++;
			if ((!isNaN(key)) && key >= 0) { // make sure the keys are numeric
...
//		console.log('Object Traits', traits);
		...
		if (traits.externalizable) {
			...
			try {
				...
				// We will attempt to read it as an object or die trying...
					...
					ALERT("Externalized object (" + traits.class + ") encountered. Any data parsed after object will most likely be incorrect.<br>Saving this file after opening will most generate a corrupted file.");
				}
			} catch (e) {
				ALERT("Unable to read externalizable data type '" + traits.class + "'  |  " + e);
				obj["??? Warning: Externalized ???"] = { value:"Unable to read externalizable data type '" + traits.class + "'", __type:'String' };
			}
		} else {
			...
			try {
				if (traits.class.indexOf("flex.") == 0) {
					...
					if (unqualifiedClassName && flex[unqualifiedClassName]) {
						var flexParser = new flex[unqualifiedClassName]();
	...
	readXML: function(ba) {
		...
		var xml = { value:ba.readUTFBytes(ref >> 1), __traits:{ type:'XML' }};
	...
	readVectorInt: function(ba) {
		...
		for (var i = 0; i < l; i++) {
			vect[i] = { value:ba.readInt(), __traits:{ type:'Integer' }};
		}
	...
	readVectorUInt: function(ba) {
		...
		var isFixed = Boolean(ba.readBoolean());
		for (var i = 0; i < l; i++) {
			vect[i] = { value:ba.readUnsignedInt(), __traits:{ type:'Integer' }};
		}
		
		var traits = { fixed:isFixed, type:'Vector.<uint>', class:'uint' };
		var val = { value:vect, __traits:traits };
		this.readObjectCache.push(val);
		return val;
	},
	...
	readVectorDouble: function(ba) {
		...
		var isFixed = Boolean(ba.readBoolean());
		for (var i = 0; i < l; i++) {
			vect[i] = { value:ba.readDouble(), __traits:{ type:'Number' }};
		}
		
		var traits = { fixed:isFixed, type:'Vector.<Number>', class:'Number' };
		var val = { value:vect, __traits:traits };
		this.readObjectCache.push(val);
		return val;
	},
	...
	readDictionary: function(ba) {
		...
		for (var i = 0; i < l; i++) {
			dict[i] = { key:this.readData(ba), value:this.readData(ba), __traits:{ type:'DictionaryItem' }};
		}
		
		var val = { value:dict, __traits:{ weakKeys:hasWeakKeys, type:'Dictionary' } };
	...
	//trace('getStringReference', ref);
		if (ref >= this.readStringCache.length) {
			...
			return { value:"??? Error: Str Ref #" + ref + " ???", __traits:{ type:'String', missingRef:ref } };
		}
	...
	//trace('getTraitReference', ref);
		if (ref >= this.readTraitsCache.length) {
			...
			return { value:"??? Error: Trait Ref #" + ref + " ???", __traits:{ type:'String', missingRef:ref } };
		}
	...
	//trace('getObjectReference', ref);
		if (ref >= this.readObjectCache.length) {
			...
			return { value:"??? Error: Obj Ref #" + ref + " ???", __traits:{ type:'String', missingRef:ref } };
		}
