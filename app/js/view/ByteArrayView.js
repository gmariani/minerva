...
var ByteArrayView = function() {
	...
	function init(el/*:Element */, node/*:Object */, input/*:Object */, callBack/*:Function */)/*:void */ {
		...
		elValue.on('keyup', function(event) {
	...
	function reset() {
		if (inputDelay) clearTimeout(inputDelay);
		elCompType = elString = elValue = 
		elOffset = elAlgorithm = ba = 
		callBackFunc = treeNode = null;
	};
	...
	function displayValue() {
		...
		var str = '';
		var hex = '';
		var offsets = '<div><div><pre title="0x0000000000 = 16">0000000000</pre>';
		var originalLen;
		...
		for (var i = 0, originalLen = l = ba.length; i < l; i++) {
			...
			if ((i + 1) % 16 == 0 && (i + 1) < l) {
				...
				s = s.slice(-10);
				offsets+= '<pre title="0x' + s + ' = ' + i + '">' + s + '</pre>';
			}
		}
	...
	function baExport(event) {
		var fileName = event.data.node.text,
			blob = new Blob([new Uint8Array(ba)], {type: 'application/octet-binary'});
		saveAs(blob, (fileName || 'bytearray')+ ".dat");
	}
	
	function baImport(event) {
		$('#overlay').addClass('show');
		$('#importBinaryWindow').addClass('show');
	}
	
	// Called from modal after file loaded
	function baImport2(buffer, file) {
	...
	function compress(event) {
		switch (elAlgorithm.val()) {
			case 'zlib' :
				...
				for "Hello World!"
				*/
				ba = new Zlib.Deflate(ba, { compressionType: Zlib.Deflate.CompressionType.FIXED }).compress();
			...
			case 'deflate' :
				...
				Zlib.Deflate.CompressionType.DYNAMIC
				*/
				ba = new Zlib.RawDeflate(ba, { compressionType: Zlib.Deflate.CompressionType.FIXED }).compress();
				if (debug) console.log('compress', ba);
				displayValue();
		...
		switch (elAlgorithm.val()) {
			...
			case 'lzma' :
				...
				LZMA.decompress(ba, function(result) {
					if (result === false) {
						Alert.show('Error decompressing via LZMA.', Alert.ERROR);
						return;
					}
					ba = result;
					if (debug) console.log('uncompress', ba);
	...
	function isLZMA(input) {
		...
		var ba2 = new ByteArray(Uint8Array(input).buffer, ByteArray.LITTLE_ENDIAN);
		
		var lc, lp, pb, d = ba2.readUnsignedByte() & 0xFF;
		if (d >= (9 * 5 * 5)) {
		...
		*/
		...
		var uncompressedSize = (((ba2.readUnsignedByte() | (ba2.readUnsignedByte() << 8)) | (ba2.readUnsignedByte() << 0x10)) | (ba2.readUnsignedByte() << 0x18));
